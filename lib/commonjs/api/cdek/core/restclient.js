...
var _api = require("../errors/api.js");
var _auth = require("../errors/auth.js");
var _http = require("../errors/http.js");
class RestClient {
  constructor(options) {
  ...
  async auth() {
    ...
    if (res.ok === false) {
      const error = new _auth.AuthError(await res.text(), {
    ...
    try {
      ...
      if (res.ok === false) {
        if (res.headers.get('Content-Type') === 'application/json') {
          const error = new _api.ApiError(await res.json(), `${res.status} ${res.statusText}, ${res.url}`);
          this.logError(init.method, target, error);
          throw error;
        } else {
          const error = new _http.HttpError('HttpError\n' + (await res.text()));
          this.logError(init.method, target, error);
          throw error;
        }
      }
