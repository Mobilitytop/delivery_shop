...
// Предполагается, что типы находятся в отдельном файле
...
const convertPackage = (packages, method) => {
  switch (method) {
    ...
    case _types.DeliveryMethodId.COURIERIST:
      return {
    ...
    case _types.DeliveryMethodId.LPOST_COURIER:
    case _types.DeliveryMethodId.LPOST_POINT:
      return {
      ...
      const requests = deliveryMethods.map(method => {
        switch (method.method) {
          case _types.DeliveryMethodId.POST:
            ...
            const postPackageData = convertPackage(packages, _types.DeliveryMethodId.POST);
          ...
          case _types.DeliveryMethodId.CDEK_DOOR:
            ...
            const cdekDoorPackageData = convertPackage(packages, _types.DeliveryMethodId.CDEK_DOOR);
          ...
          case _types.DeliveryMethodId.CDEK_POINT:
            if (!CDEKClient || !cdekConfig) return Promise.resolve(null);
            const cdekPointPackageData = convertPackage(packages, _types.DeliveryMethodId.CDEK_POINT);
            return CDEKClient.calculatorByTariff({
              ...cdekConfig.request,
              tariff_code: 136,
              from_location: {
          ...
          case _types.DeliveryMethodId.LPOST_COURIER:
            ...
            const lpostCourierPackageData = convertPackage(packages, _types.DeliveryMethodId.LPOST_COURIER);
          ...
          case _types.DeliveryMethodId.LPOST_POINT:
            ...
            const lpostPointPackageData = convertPackage(packages, _types.DeliveryMethodId.LPOST_POINT);
            return LPostClient.getServicesCalc({
              fromWarehouseId: method.data.fromWarehouseId,
      ...
      deliveryMethods.forEach((method, index) => {
        ...
        switch (method.method) {
          ...
          case _types.DeliveryMethodId.COURIERIST:
            if (response?.price) {
              options.push({
                id: _types.DeliveryMethodId.COURIERIST,
                title: 'Курьерист',
                cost: response.price,
          ...
          case _types.DeliveryMethodId.LPOST_COURIER:
            if (response?.sumCost) {
              options.push({
                id: _types.DeliveryMethodId.LPOST_COURIER,
                title: 'Л-Пост (курьер)',
                cost: response.sumCost,
                duration: response.possibleDelivDates?.length ? `${response.possibleDelivDates[0].intervals[0].timeFrom}-${response.possibleDelivDates[0].intervals[0].timeTo} (${response.possibleDelivDates[0].dateDelive})` : `${response.dayLogistic} дн.`
              });
            }
            break;
          case _types.DeliveryMethodId.LPOST_POINT:
            if (response?.sumCost) {
              options.push({
                id: _types.DeliveryMethodId.LPOST_POINT,
                title: 'Л-Пост (до ПВЗ)',
                cost: response.sumCost,
